# To check if Ind and fixed seed simulation have similar delivery times
...
if __name__ == "__main__":
    ...
    simulation_delivery_times, T_list, micro_T_list, fidelity_list = train_and_sim.simulate_policy_w_fidelity()
    T = T_list
    print(f'Ind. sim. delivery times : {np.average(T)}')
    fid = [float(x) for x in fidelity_list]
    print(f'Ind. sim. fidelity : {np.average(fid)}')