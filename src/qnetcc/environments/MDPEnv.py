...
class Environment(Env_root, actions, history_obs):
    ...
    def __init__(self, pos_of_agent, nodes, t_cut, p, p_s, max_moves = 10**5, cc_effects=1):
        '''
        ...
        Env_root.__init__(self, nodes, t_cut, p, p_s) # constructor for the root environement
        ...
        # for having or not having CC effects; if cc_effects = 0, then the CC effects are not included in the environment
        self.cc_effects = cc_effects
        assert self.cc_effects == 0 or self.cc_effects == 1, "cc_effects can only be 0 or 1"
    ...
    def reset(self, seed=None):
        ...
        self.quantum_network = Quantum_network(self.nodes,self.t_cut,self.p,self.p_s)
    ...
    def give_reward(self):
        ...
        When CC effect = 1, wait for end-to-end verification
        ...
        When CC effect = 0, the episode terminates when the end-to-end link is established or when the maximum number of moves is reached.
        """


        if self.cc_effects == 1:
            # With verification 
            reward = 0
            ...
            if self.quantum_network.A_B_entangled():
                ...
                reward += -1
                self.consec_A_B_ent_time_steps = 0
        elif self.cc_effects == 0:
            reward = -1
            terminated = False
            if self.quantum_network.A_B_entangled():
                reward = 0
                terminated = True
                assert self.quantum_network.micro_time_slot%2 == 1
                # assert self.action_time_step == 'swap'
            elif self.quantum_network.micro_time_slot > self.max_moves:
                terminated = True
                reward += -1
