# To plot the corresponding fidelity of each point of Fig 7. 
...
def load_RL_fid_data(nodes, t_cut, p, p_s, MC_runs, trained_version=203):
    ...
    partial_data = []
    sim_dat_path = save_path+f'/env_cc_a_alt_o_hist/fid_datInd{trained_version}/fid_dat_cc_fidelity_'+f'n_{nodes}_t_cut_{t_cut_cc}_p_{p:.02f}_p_s_{p_s:.02f}_v{version}.pkl'
    # If it exists...
    ...
    assert not np.isnan(data).any(), f"Array contains NaN at nodes: {nodes}, t_cut: {t_cut_cc}, p: {p:.02f}, p_s: {p_s:.02f}"
...
def load_swap_asap_fid_data(setting, nodes, t_cut, p, p_s, MC_runs):
    ...
    if setting == 'swap-asap (cc effects)':
        sim_dat_path = save_path+f'/swap_asap_cc/fid_dat/fid_dat_cc_fidelity_'+f'n_{nodes}_t_cut_{t_cut_cc}_p_{p:.02f}_p_s_{p_s:.02f}_v{version}.pkl'
    elif setting == 'swap-asap (no cc effects)':
        sim_dat_path = save_path+f'/swap_asap/fid_dat/fid_dat_cc_fidelity_'+f'n_{nodes}_t_cut_{t_cut_cc}_p_{p:.02f}_p_s_{p_s:.02f}_v{version}.pkl'
    elif setting == 'swap-asap (predictive)':
        sim_dat_path = save_path+f'/swap_asap_with_pred/fid_dat/fid_dat_cc_fidelity_'+f'n_{nodes}_t_cut_{t_cut_cc}_p_{p:.02f}_p_s_{p_s:.02f}_v{version}.pkl'
...
def combinedFideliyComparison(nodes, t_cut, problist, samples):
    current_path = os.getcwd()
    data_path = os.path.join(current_path, '../..', 'data')
    best_agent_directory = data_path+f'/global_agent_swap_sum/env_cc_a_alt_o_hist/bestAgents/'
    RLMeanFidList = []
    SAWBMeanFidList = []
    SwapAsapPredMeanFidList = []
    stdRLMeanFidList = []
    stdSAWBMeanFidList = []
    stdSwapAsapPredMeanFidList = []
    for (p_s, p) in problist:
        ...
        assert len(RLFid) == samples
        stdRLMeanFidList.append(np.std(RLFid)/np.sqrt(samples))
        SwapAsapWBFid = load_swap_asap_fid_data('swap-asap (cc effects)', nodes, t_cut, p, p_s, samples)
        SAWBMeanFidList.append(np.average(SwapAsapWBFid))
        assert len(SwapAsapWBFid) == samples
        stdSAWBMeanFidList.append(np.std(SwapAsapWBFid)/np.sqrt(samples))
        SwapAsapPredFid = load_swap_asap_fid_data('swap-asap (predictive)', nodes, t_cut, p, p_s, samples)
        SwapAsapPredMeanFidList.append(np.average(SwapAsapPredFid))
        assert len(SwapAsapPredFid) == samples
        stdSwapAsapPredMeanFidList.append(np.std(SwapAsapPredFid)/np.sqrt(samples))
    ...
    # Bar plots: each shifted slightly so they don’t overlap
    ax.bar(x - width, RLMeanFidList, width, label='RL agent', yerr= stdRLMeanFidList,error_kw={
    ...
    ax.bar(x, SAWBMeanFidList, width, label='WB swap-asap', yerr=stdSAWBMeanFidList, error_kw={
           'capsize': 5,      # size of the caps in points
           'capthick': 2,     # thickness of the cap lines
           'elinewidth': 1.5, # thickness of the error bar line itself
           'ecolor': 'black'  # color of the error bars
       }, color='#338000ff')
    ax.bar(x + width, SwapAsapPredMeanFidList, width, label='Predictive swap-asap', yerr=stdSwapAsapPredMeanFidList, error_kw={
           'capsize': 5,      # size of the caps in points
           'capthick': 2,     # thickness of the cap lines
           'elinewidth': 1.5, # thickness of the error bar line itself
           'ecolor': 'black'  # color of the error bars
    ...
    print('Ps = ', p_s)
    print('Max standard deviations RL: ', np.max(stdRLMeanFidList))
    print('Max standard deviations WB: ', np.max(stdSAWBMeanFidList))
    print('Max standard deviations Predictive: ', np.max(stdSwapAsapPredMeanFidList))
    ...
    if not os.path.exists(save_directory):
        os.makedirs(save_directory) 
    prob_str = "_".join(str(val) for tup in problist for val in tup)
    plt.savefig(save_directory+f'/combFigFidCompNodeswErr{nodes}Tcut{t_cut}Prob{prob_str}.pdf',dpi=1200)
    plt.savefig(save_directory+f'/combFigFidCompNodeswErr{nodes}Tcut{t_cut}Prob{prob_str}.jpg',dpi=1200)
...
if __name__ == "__main__":
    ...
    current_path = os.getcwd()
    data_path = os.path.join(current_path, '../..', 'data')
    save_directory = data_path+f'/global_agent_swap_sum/env_cc_a_alt_o_hist/bestAgents/'
